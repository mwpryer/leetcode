...
var findAllPeople = function (n, meetings, firstPerson) {
  ...
  for (const time of times) {
    ...
    const groupSecrets = new Set()
    for (const [x, y] of groups.get(time)) {
      if (!neighbours.has(x)) neighbours.set(x, [])
      neighbours.get(x).push(y)
      if (!neighbours.has(y)) neighbours.set(y, [])
      neighbours.get(y).push(x)
      if (secrets.has(x)) groupSecrets.add(x)
      if (secrets.has(y)) groupSecrets.add(y)
    }

    const stack = Array.from(groupSecrets)
    while (stack.length > 0) {
      const curr = stack.pop()
      secrets.add(curr)
      for (const neighbour of neighbours.get(curr)) {
        if (!secrets.has(neighbour)) {
          secrets.add(neighbour)
