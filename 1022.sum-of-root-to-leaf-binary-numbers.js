...
var sumRootToLeaf = function (root) {
  ...
  while (queue.length > 0) {
    ...
    for (let i = 0; i < n; i++) {
      const [curr, parentSum] = queue.shift()
      const sum = parentSum * 2 + curr.val
      if (!curr.left && !curr.right) res += sum
      if (curr.left) queue.push([curr.left, sum])
      if (curr.right) queue.push([curr.right, sum])
    }
  }
  return res
}
